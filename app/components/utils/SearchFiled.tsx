...
const SearchFiled = (props: SearchFiledProps) => {
    ...
    const labelClassName = useMemo(() => {
        ...
        ${isFocus ? `bottom-[95%] left-0 text-secondary`
    ...
    return (
        <div className={`flex flex-col justify-center items-center py-0.5 px-3 w-full gap-2 ${props.className || ""}`}>
            <div className="flex flex-row gap-2 w-full justify-center items-center relative">
                ...
                <input
                    {...props.inputProps}
                    className="py-0.5 px-1 dark:text-white bg-normal border h-fit rounded-sm w-full focus:border-none focus:outline-solid focus:outline-2 dark:border-gray-300 dark:hover:border-white border-gray-700 hover:border-gray-900 focus:outline-secondary"
                    id={Id}
                    value={props.value}
                    onFocus={onFocus}
                    onBlur={onBlur}
