...
export default function Main({ createdAt, category, authorId, blogName, post }: IProps) {
  const { current: html } = useRef(mdParser(post))

  const [isFullScreen, setIsFullScreen] = useState(false)

  return (
    <Container className={`bg-white transition-all ease-in-out p-4 ${isFullScreen ? "absolute z-50 top-0 left-0 right-0 bottom-0 w-full h-max" : "border border-gray-300 rounded-md shadow-lg make-width-fit"}`}>
